...
public class PermissionDataUtil {
	...
	public static SysPermission intelligentProcessData(SysPermission permission) {
		...
		// 组件
		if (OConvertUtil.isNotEmpty(permission.getComponent())) {
		...
		// 请求URL
		if (OConvertUtil.isNotEmpty(permission.getUrl())) {
		...
		// 一级菜单默认组件
		if (0 == permission.getMenuType() && OConvertUtil.isEmpty(permission.getComponent())) {
			// 一级菜单默认组件
