...
import com.haso.common.util.YouBianCodeUtil;
import com.haso.system.entity.SysDepart;
import com.haso.system.mapper.SysDepartMapper;
import com.haso.system.model.DepartIdModel;
import com.haso.system.model.SysDepartTreeModel;
import com.haso.system.util.FindsDepartsChildrenUtil;
...
public class SysDepartService extends ServiceImpl<SysDepartMapper, SysDepart> {
	...
	public void saveDepartData(SysDepart sysDepart, String username) {
		if (sysDepart != null && username != null) {
			...
			sysDepart.setId(ConvertUtil.getInteger(s));
			// 先判断该对象有无父级ID,有则意味着不是最高级,否则意味着是最高级
			// 获取父级ID
			String parentId = ConvertUtil.getString(sysDepart.getParentId());
	...
	public void deleteBatchWithChildren(List<String> ids) {
		...
		for(String id: ids) {
			idList.add( ConvertUtil.getInteger(id));
			this.checkChildrenExists(id, idList.stream().map(ConvertUtil::getString).collect(Collectors.toList()));
		}
	...
	private void checkChildrenExists(String id, List<String> idList) {	
		LambdaQueryWrapper<SysDepart> query = new LambdaQueryWrapper<SysDepart>();
		query.eq(SysDepart::getParentId, ConvertUtil.getInteger(id));
		List<SysDepart> departList = this.list(query);
		if(departList != null && departList.size() > 0) {
			for(SysDepart depart : departList) {
				idList.add(ConvertUtil.getString(depart.getId()));
				this.checkChildrenExists(ConvertUtil.getString(depart.getId()), idList);
			}
		}
	}

	public List<SysDepart> queryUserDeparts(String userId) {
		return baseMapper.queryUserDeparts(ConvertUtil.getInteger(userId));
	}
