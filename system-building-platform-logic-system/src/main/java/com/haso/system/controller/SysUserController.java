...
import com.haso.system.entity.SysUserRole;
import com.haso.system.model.DepartIdModel;
import com.haso.system.model.SysUserSysDepartModel;
import com.haso.system.service.*;
import com.haso.system.shiro.vo.SysDepartUsersVO;
import com.haso.system.shiro.vo.SysUserRoleVO;
...
public class SysUserController {
	...
	public Result<IPage<SysUser>> queryPageList(SysUser user, @RequestParam(name="pageNo", defaultValue="1") Integer pageNo,
		...
		IPage<SysUser> pageList = sysUserService.selPage(page, user, column, order);

		//批量查询用户的所属部门
        //step.1 先拿到全部的 useids
        //step.2 通过 useids，一次性查询用户的所属部门名字
//        List<Integer> userIds = pageList.getRecords().stream().map(SysUser::getId).collect(Collectors.toList());
//        if(userIds!=null && userIds.size()>0){
//            Map<Integer, String>  useDepNames = sysUserService.getDepNamesByUserIds(userIds);
...
//        }
	...
	public Result<SysUser> add(@RequestBody JSONObject jsonObject) {
		...
		try {
            ...
            user.setCreateTime(new Timestamp(System.currentTimeMillis()));//设置创建时间
	...
	public Result<SysUser> edit(@RequestBody JSONObject jsonObject) {
		Result<SysUser> result = new Result<SysUser>();
		try {
			SysUser sysUser = sysUserService.getById(ConvertUtil.getInteger(jsonObject.getString("id")));
			sysBaseAPI.addLog("ユーザーを編集，id： " +jsonObject.getString("id") , CommonConstant.LOG_TYPE_2, 2);
			if(sysUser==null) {
				result.error500("対応するエンティティが見つかりませんでした。");
			}else {
				SysUser user = JSON.parseObject(jsonObject.toJSONString(), SysUser.class);
	...
	public Result<?> deleteBatch(@RequestParam(name="ids",required=true) String ids) {
		sysBaseAPI.addLog("一括削除ユーザ， ids： " +ids ,CommonConstant.LOG_TYPE_2, 3);
		this.sysUserService.deleteBatchUsers(ids);
		return Result.ok("一括削除に成功しました。");
	}
	...
	public Result<SysUser> frozenBatch(@RequestBody JSONObject jsonObject) {
		...
		try {
			...
			for (String id : arr) {
				if(OConvertUtil.isNotEmpty(id)) {
					this.sysUserService.update(new SysUser().setStatus(Integer.parseInt(status)),
							new UpdateWrapper<SysUser>().lambda().eq(SysUser::getId,Integer.parseInt(id)));
				}
			}
		} catch (Exception e) {
			log.error(e.getMessage(), e);
    ...
    public Result<SysUser> queryById(@RequestParam(name = "id", required = true) Integer id) {
        Result<SysUser> result = new Result<SysUser>();
    ...
    public Result<List<Integer>> queryUserRole(@RequestParam(name = "userid", required = true) String userid) {
        ...
        List<SysUserRole> userRole = sysUserRoleService.list(new QueryWrapper<SysUserRole>().lambda().eq(SysUserRole::getUserId, ConvertUtil.getInteger(userid)));
    ...
    public Result<List<DepartIdModel>> getUserDepartsList(@RequestParam(name = "userId", required = true) Integer userId) {
...
//    }
	...
	public Result<Collection<SysUser>> queryByIds(@RequestParam String userIds) {
        ...
        Integer[] ints = new Integer[userId.length];
        for(int i=0;i<userId.length;i++){
            ints[i] = Integer.parseInt(userId[i]);
        }
		Collection<Integer> idList = Arrays.asList(ints);
    ...
    public Result<IPage<SysUser>> departUserList(@RequestParam(name="pageNo", defaultValue="1") Integer pageNo,
        ...
        //部门id为空时，查询我的部门下所有用户
        if(OConvertUtil.isEmpty(depId)){
            LoginUser user = (LoginUser) SecurityUtils.getSubject().getPrincipal();
            int userIdentity = user.getIdentity() != null?user.getIdentity():CommonConstant.USER_IDENTITY_1;
            if(OConvertUtil.isNotEmpty(userIdentity) && userIdentity == CommonConstant.USER_IDENTITY_2 ){
...
//            }
    ...
    public Result<?> queryByOrgCodeForAddressList(
        ...
        for (SysUserSysDepartModel item : list) {
            String userId = ConvertUtil.getString(item.getSysUser().getId());
            // userId
    ...
    public Result<Map<String,Object>> getCurrentUserDeparts() {
        ...
        try {
            ...
            List<SysDepart> list = this.sysDepartService.queryUserDeparts(ConvertUtil.getString(sysUser.getId()));
...
//		Object code = redisUtil.get(phone);
		...
		if(OConvertUtil.isEmpty(password)){
            password = RandomUtil.randomString(8);
        }
		String email = jsonObject.getString("email");
		SysUser sysUser1 = sysUserService.getUserByName(username);
		if (sysUser1 != null) {
			result.setMessage("登録済みのユーザー名です");
			result.setSuccess(false);
			return result;
		}
		...
		if(OConvertUtil.isNotEmpty(email)){
...
//		}
	...
	public Result<Map<String, Object>> querySysUser(SysUser sysUser) {
		...
		Result<Map<String, Object>> result = new Result<Map<String, Object>>();
		Map<String, Object> map = new HashMap<String, Object>();
		if (OConvertUtil.isNotEmpty(phone)) {
		...
		if (OConvertUtil.isNotEmpty(username)) {
...
//		redisUtil.set(phone, smscode);
	...
	public Result<SysUser> passwordChange(@RequestParam(name="username")String username,
        ...
        Result<SysUser> result = new Result<SysUser>();
        if(OConvertUtil.isEmpty(username) || OConvertUtil.isEmpty(password) || OConvertUtil.isEmpty(smscode)  || OConvertUtil.isEmpty(phone) ) {
