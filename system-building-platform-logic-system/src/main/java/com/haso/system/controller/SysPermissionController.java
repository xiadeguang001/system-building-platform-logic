...
import com.haso.common.util.JwtUtil;
import com.haso.common.util.MD5Util;
import com.haso.common.util.OConvertUtil;
import com.haso.system.entity.SysDepartPermission;
import com.haso.system.entity.SysPermission;
import com.haso.system.entity.SysPermissionDataRule;
import com.haso.system.entity.SysRolePermission;
import com.haso.system.model.SysPermissionTree;
import com.haso.system.model.TreeModel;
import com.haso.system.service.SysDepartPermissionService;
import com.haso.system.service.SysPermissionDataRuleService;
import com.haso.system.service.SysPermissionService;
import com.haso.system.service.SysRolePermissionService;
import com.haso.system.util.PermissionDataUtil;
	...
	public Result getSystemSubmenuBatch(@RequestParam("parentIds") String parentIds) {
		try {
			...
			for (SysPermission item : list) {
				String pid = ConvertUtil.getString(item.getParentId());
...
//				SysPermission indexMenu = sysPermissionService.list(new LambdaQueryWrapper<SysPermission>().eq(SysPermission::getName,"タスク管理")).get(0);
...
//			}
	...
	public Result<SysPermission> delete(@RequestParam(name = "id", required = true) Integer id) {
		Result<SysPermission> result = new Result<>();
		try {
	...
	public Result<Map<String, Object>> queryTreeList() {
		...
		try {
			...
			for (SysPermission sysPer : list) {
				ids.add(ConvertUtil.getString(sysPer.getId()));
			}
	...
	public Result<List<String>> queryRolePermission(@RequestParam(name = "roleId", required = true) Integer roleId) {
	...
	public Result<String> saveRolePermission(@RequestBody JSONObject json) {
		...
		try {
			...
			result.success("保存成功");
			log.info("======権限付与成功=====" + (System.currentTimeMillis() - start) + ":ミリ秒時間がかかる");
		} catch (Exception e) {
			result.error500("権限付与失敗");
			log.error(e.getMessage(), e);
		}
		return result;
	}
	...
	private void getTreeList(List<SysPermissionTree> treeList, List<SysPermission> metaList, SysPermissionTree temp) {
		for (SysPermission permission : metaList) {
			String tempPid = ConvertUtil.getString(permission.getParentId());
			SysPermissionTree tree = new SysPermissionTree(permission);
			if (temp == null && OConvertUtil.isEmpty(tempPid)) {
				treeList.add(tree);
				if (!tree.getIsLeaf()) {
	...
	private void getPermissionJsonArray(JSONArray jsonArray, List<SysPermission> metaList, JSONObject parentJson) {
		for (SysPermission permission : metaList) {
			...
			if (parentJson == null && OConvertUtil.isEmpty(tempPid)) {
				jsonArray.add(json);
				if (!permission.isLeaf()) {
					getPermissionJsonArray(jsonArray, metaList, json);
				}
			} else if (parentJson != null && OConvertUtil.isNotEmpty(tempPid) && tempPid.equals(parentJson.getString("id"))) {
	 ...
	 */
		...
		if (permission.getMenuType().equals(CommonConstant.MENU_TYPE_2)) {
			...
			if (OConvertUtil.isNotEmpty(permission.getComponentName())) {
			...
			if (OConvertUtil.isEmpty(permission.getParentId())) {
				// 一级菜单跳转地址
				json.put("redirect", permission.getRedirect());
				if (OConvertUtil.isNotEmpty(permission.getIcon())) {
	...
	private String urlToRouteName(String url) {
		if (OConvertUtil.isNotEmpty(url)) {
			if (url.startsWith("/")) {
				url = url.substring(1);
			}
			url = url.replace("/", "-");
	...
	public Result<List<SysPermissionDataRule>> getPermRuleListByPermId(SysPermissionDataRule sysPermissionDataRule) {
		List<SysPermissionDataRule> permRuleList = sysPermissionDataRuleService.getPermRuleListByPermId(ConvertUtil.getString(sysPermissionDataRule.getPermissionId()));
