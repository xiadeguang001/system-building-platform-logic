...
import com.haso.system.service.SysRoleService;
...
public class SysRoleController {
	...
	public Result<SysRole> edit(@RequestBody SysRole role) {
		...
		if(sysrole==null) {
			result.error500("対応するエンティティが見つかりません");
		}else {
	...
	public Result<?> delete(@RequestParam(name="id",required=true) String id) {
		sysRoleService.deleteRole(ConvertUtil.getInteger(id));
		return Result.ok("役割削除成功");
	}
	...
	public Result<SysRole> deleteBatch(@RequestParam(name="ids",required=true) String ids) {
		Result<SysRole> result = new Result<SysRole>();
		if(OConvertUtil.isEmpty(ids)) {
			result.error500("未選択役割");
		}else {
			sysRoleService.deleteBatchRole(Arrays.stream(ids.split(",")).map(ConvertUtil::getInteger).toArray(Integer[]::new));
			result.success("役割削除成功");
		}
		return result;
	}
	...
	public Result<?> loadDatarule(@PathVariable("permissionId") String permissionId,@PathVariable("roleId") String roleId) {
		...
		if(list==null || list.size()==0) {
			...
			if(sysRolePermission==null) {
				...
				if(OConvertUtil.isNotEmpty(drChecked)) {
	...
	public Result<Map<String,Object>> queryTreeList(HttpServletRequest request) {
		...
		try {
			...
			query.eq(SysPermission::getDelFlag, CommonConstant.DEL_FLAG_0)
					.eq(SysPermission::isHidden,false);
	...
	private void getTreeModelList(List<TreeModel> treeList, List<SysPermission> metaList, TreeModel temp) {
		for (SysPermission permission : metaList) {
			...
			TreeModel tree = new TreeModel(ConvertUtil.getString(permission.getId()), tempPid, permission.getName(),permission.getRuleFlag(), permission.isLeaf());
