...
import com.haso.common.constant.CommonSendStatus;
...
import com.haso.system.module.message.WebSocket;
import com.haso.system.service.SysAnnouncementSendService;
import com.haso.system.service.SysAnnouncementService;
...
public class SysAnnouncementController {
    ...
    public Result<SysAnnouncement> delete(@RequestParam(name="id",required=true) Integer id) {
        Result<SysAnnouncement> result = new Result<SysAnnouncement>();
    ...
    public Result<SysAnnouncement> deleteBatch(@RequestParam(name="ids",required=true) String ids) {
        ...
        if(ids==null || "".equals(ids.trim())) {
            ...
            for(int i=0;i<id.length;i++) {
                SysAnnouncement announcement = sysAnnouncementService.getById(ConvertUtil.getInteger(id[i]));
    ...
    public Result<Map<String,Object>> listByUser(HttpServletRequest request) {
        ...
        result.setSuccess(true);
        String currentUserName = JwtUtil.getUsername(request.getHeader("X-Access-Token"));
        SysUser sysUser = sysUserService.getUserByName(currentUserName);
        String userId = ConvertUtil.getString(sysUser.getId());
        ...
        if(anntIds!=null&&anntIds.size()>0) {
            querySaWrapper.notIn(SysAnnouncement::getId, anntIds.stream().map(ConvertUtil::getInteger).collect(Collectors.toList()));
        }
        ...
        if(announcements.size()>0) {
            for(int i=0;i<announcements.size();i++) {
                ...
                announcementSend.setUserId(ConvertUtil.getInteger(userId));
    ...
    public Result<SysAnnouncement> queryById(@RequestParam(name="id",required=true) Integer id) {
        Result<SysAnnouncement> result = new Result<SysAnnouncement>();
        SysAnnouncement sysAnnouncement = sysAnnouncementService.getById(id);
        if(sysAnnouncement==null) {
            result.error500("未找到对应实体");
    ...
    public Result<SysAnnouncement> doReleaseData(@RequestParam(name="id",required=true) Integer id, HttpServletRequest request) {
        Result<SysAnnouncement> result = new Result<SysAnnouncement>();
        SysAnnouncement sysAnnouncement = sysAnnouncementService.getById(id);
        if(sysAnnouncement==null) {
            result.error500("未找到对应实体");
    ...
    public Result<SysAnnouncement> doReovkeData(@RequestParam(name="id",required=true) Integer id, HttpServletRequest request) {
        Result<SysAnnouncement> result = new Result<SysAnnouncement>();
        SysAnnouncement sysAnnouncement = sysAnnouncementService.getById(id);
        if(sysAnnouncement==null) {
            result.error500("未找到对应实体");
