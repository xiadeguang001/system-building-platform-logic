...
import com.haso.common.constant.DataBaseConstant;
import com.haso.common.system.vo.SysPermissionDataRuleModel;
import com.haso.common.util.DataAutorUtils;
...
//    private static ApplicationContextUtil applicationContextUtil;
        ...
        for (int i = 0; i < origDescriptors.length; i++) {
            ...
            try {
                ...
                if (null != value && value.toString().startsWith(COMMA) && value.toString().endsWith(COMMA)) {
                    ...
                    final String field = OConvertUtil.camelToUnderline(name);
                    if(vals.length>1) {
                        queryWrapper.and(j -> {
    ...
    public static void doMultiFieldsOrder(QueryWrapper<?> queryWrapper,Map<String, String[]> parameterMap) {
        ...
        if (OConvertUtil.isNotEmpty(column) && OConvertUtil.isNotEmpty(order)) {
            ...
            if (order.toUpperCase().indexOf(ORDER_TYPE_ASC)>=0) {
                queryWrapper.orderByAsc(OConvertUtil.camelToUnderline(column));
            } else {
                queryWrapper.orderByDesc(OConvertUtil.camelToUnderline(column));
            }
        }
    }
    ...
    private static void addEasyQuery(QueryWrapper<?> queryWrapper, String name, QueryRuleEnum rule, Object value) {
        ...
        name = OConvertUtil.camelToUnderline(name);
        ...
        if(rule.equals(QueryRuleEnum.IN) && ! propertyType.equals(String.class)) {
            ...
            if (propertyType.equals(String.class)) {
                ...
                if(dateStr.length()==10){
                    addEasyQuery(queryWrapper, name, rule, DateUtil.str2Date(dateStr,DateUtil.DATE_SDF));
                }else{
                    addEasyQuery(queryWrapper, name, rule, DateUtil.str2Date(dateStr,DateUtil.DATETIME_FORMAT));
                }
            }else {
    ...
    public static String getSingleQueryConditionSql(String field,String alias,Object value,boolean isString) {
        ...
        field =  alias+ OConvertUtil.camelToUnderline(field);
    ...
    public static String installAuthJdbc(Class<?> clazz) {
        ...
        for (int i = 0; i < origDescriptors.length; i++) {
            ...
            if(ruleMap.containsKey(name)) {
                ...
                String filedSql = getSingleSqlByRule(rule, OConvertUtil.camelToUnderline(name), value,isString);
